...
public class Ex04Stream {
  public static void main(String[] args) {
    ...
    // Stream 을 통하여 데이터를 모은다.
    Stream<Student> stream = Stream.of( //이 원소들을 가지고 만들겠다.
    ...
    //stream을 정렬하는 method 적용하고 비교하는 객체를 Comparator.comparing
    /*stream.sorted(Comparator.comparing(new Function<Student, Integer>() {
      ...
      //반별로 묶은 것을 가지고 반을 기준으로 (naturalOrder)오름차순으로 정렬한다.
    }).thenComparing(Comparator.naturalOrder())).forEach(new Consumer<Student>() {
      @Override
      public void accept(Student student) {
        System.out.println(student); // 정렬 후 각각에 대하여 출력한다.(forEach)
      }
    });*/

    // Optional 객체 사용하여 totalScore 점수가 가장 큰 사람 구하기
    // 함수형 인터페이스 Function에 매개변수를 2개 받기 위해 만든 함수 =>BinaryOperator
   ...
   /*System.out.println(Arrays.toString(stream.toArray(Student[]::new)));
    ...
    System.out.println(stream.collect(Collectors.toCollection()));*/
    ...
    System.out.println(stream.collect(Collectors.maxBy(Comparator.comparingInt(
        new ToIntFunction<Student>() {
      @Override
      public int applyAsInt(Student s) {
        return s.getTotalScore();
